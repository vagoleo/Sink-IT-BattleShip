    ...
    for(let i = 0; i < size; i++){
        ...
        for(let j = 0; j < size; j++){
            ...
            if(i > 0 && j > 0){
                ...
                if(id == 'salvo' && !(cell.classList.contains('firedSalvo'))){
                    cell.addEventListener('click', function(event) {app.aim(event)});
                    cell.classList.add('hover');
                    cell.classList.add('aimable');
                } 
            }
      ...
      if(ship.dataset.orientation == 'horizontal'){
        ...
        for(let i = 1; i < ship.dataset.length;i++){
            ...
            if(document.getElementById(cellId).className.search(/busy-cell/) != -1){
                document.querySelector("#display p").innerText = 'Be careful'
                return
            }
        }
      } else{
